...
listOfTables = cursor.execute(
    ...
    WHERE type = 'table' AND name = 'pest_monitoring';
    """
).fetchall()
...
# If the pest_monitoring table already
# exists then delete it and create a new one
else:
...
insert_records = """
    ...
    (time, site_id, air_temperature_c, relative_humidity_pct,
    leaf_wetness_0_1, light_lux, vibration_level,
    pest_trap_count, status, alert_triggered, alert_pest_action,
    alert_pest_outbreak, alert_disease_moderate,
    alert_disease_high, wx_rain_mm_hr) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    """
...
# A blank cell just contains nothing and using the
#  'database_test.py' you can find these cells
